	...
	public static void addUsers(UsersDtls u, Part part, ServletContext ctx) throws Exception {
	       Connection con = DbConnect.Connect();
	        PreparedStatement ps = con
	                .prepareStatement("INSERT into user(firstname,lastname,password,email,contact,city,address) VALUES(?,?,?,?,?,?,?)",
	        ...
	        ps = con.prepareStatement("update user set img=? where uid=?");
	 ...
	 public static List<UsersDtls> getUsers(boolean sort, boolean desc) throws Exception {
	        ...
	        Connection con = DbConnect.Connect();
	        ResultSet rs = con.createStatement().executeQuery("SELECT * FROM user order by uid");
	         while (rs.next()) {
	 ...
	 public UsersDtls getUserById(int uid) {
	    ...
	    public static void updateUsers(UsersDtls u) throws Exception {
	        ...
	        PreparedStatement ps = con
	                .prepareStatement("UPDATE user set firstname=?,lastname=?,email=?,contact=?,city=?,address=? where uid=?");
	        ps.setString(1, u.getFirstname());
	        ps.setString(2, u.getLastname());
	  ...
	  //update Image
	    public static void updateImage(UsersDtls u) throws Exception {
	        ...
	        PreparedStatement ps = con
	                .prepareStatement("UPDATE user set img=? where uid=?");
